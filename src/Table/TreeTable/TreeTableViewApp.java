...
public class TreeTableViewApp extends Application {
	...
	public Parent createContent() {
		...
		dataColumn.setCellValueFactory(new TreeItemPropertyValueFactory<Inventory, String>("data"));

		final TreeTableColumn<Inventory, String> notesColumn = new TreeTableColumn<>("Notes (editable)");
		final Callback<TreeTableColumn<Inventory, String>, TreeTableCell<Inventory, String>> notes = TextFieldTreeTableCell.<Inventory> forTreeTableColumn();
		notesColumn.setEditable(true);
		notesColumn.setMinWidth(150);
		notesColumn.setCellValueFactory(new TreeItemPropertyValueFactory<Inventory, String>("notes"));
		notesColumn.setCellFactory(notes);

		final TreeTableView<Inventory> treeTableView = new TreeTableView<>(getData());
