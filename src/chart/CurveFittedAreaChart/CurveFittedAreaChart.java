...
public class CurveFittedAreaChart extends AreaChart<Number, Number> {
	...
	private static void smooth(ObservableList<PathElement> strokeElements, ObservableList<PathElement> fillElements) {
		...
		for (int i = 1; i < points.length; i++) {
			final int ci = i - 1;
			strokeElements.add(new CubicCurveTo(firstControls[ci].getX(), firstControls[ci].getY(), secondControls[ci].getX(), secondControls[ci].getY(), points[i].getX(), points[i].getY()));
			fillElements.add(new CubicCurveTo(firstControls[ci].getX(), firstControls[ci].getY(), secondControls[ci].getX(), secondControls[ci].getY(), points[i].getX(), points[i].getY()));
		}
		// end the paths
	...
	public static Pair<Point2D[], Point2D[]> curveControlPoints(Point2D[] data) {
		...
		if (n == 1) { // Special case: Bezier curve should be a straight line.
			...
			// 3P1 = 2P0 + P3
			firstControlPoints[0] = new Point2D((2 * data[0].getX() + data[1].getX()) / 3, (2 * data[0].getY() + data[1].getY()) / 3);

			secondControlPoints = new Point2D[1];
			// P2 = 2P1 鈥� P0
			secondControlPoints[0] = new Point2D(2 * firstControlPoints[0].getX() - data[0].getX(), 2 * firstControlPoints[0].getY() - data[0].getY());
		...
		for (int i = 0; i < n; ++i) {
			...
			if (i < n - 1) {
				secondControlPoints[i] = new Point2D(2 * data[i + 1].getX() - x[i + 1], 2 * data[i + 1].getY() - y[i + 1]);
			} else {
				secondControlPoints[i] = new Point2D((data[n].getX() + x[n - 1]) / 2, (data[n].getY() + y[n - 1]) / 2);
			}
		}
		return new Pair<Point2D[], Point2D[]>(firstControlPoints, secondControlPoints);
	}
